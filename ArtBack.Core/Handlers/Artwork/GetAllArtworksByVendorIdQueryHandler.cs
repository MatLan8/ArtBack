...
public class GetAllArtworksByVendorIdQueryHandler(ArtDbContext dbContext) : IRequestHandler<GetAllArtworksByVendorIdQuery, List<ArtworkDto>>
{
    public async Task<List<ArtworkDto>> Handle(GetAllArtworksByVendorIdQuery request, CancellationToken cancellationToken)
    {
        return await dbContext.Artworks
            .AsNoTracking()
            .Where(a => !a.isDeleted && a.VendorId == request.VendorId)
